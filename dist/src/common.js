...
const projection_1 = require("./core/projection");
...
function escapeRegex(str) {
    return str.replace(exports.metaRegex, '\\$&');
}
...
exports.hasMongoUpdateOperator = hasMongoUpdateOperator;
...
exports.extractUpdateFields = extractUpdateFields;
// Options parsing
// ===============
...
function processOptions(collection, opts) {
    if (opts) {
        const fields = opts.fields;
        if (fields) {
            const f = projection_1.default.resolve(collection.def.projections, fields);
            if (f !== fields) {
                const newOpts = {};
...
function extractAuthorization(opts) {
    ...
    if (auth) {
        delete opts.auth;
        return auth === true ? tyr_1.default.local.user : auth;
    }
    const tyrOpts = opts.tyranid;
    if (tyrOpts) {
        delete opts.tyranid;
        if (tyrOpts.secure) {
            return tyrOpts.subject || tyrOpts.user || tyr_1.default.local.user;
        }
    }
    //return undefined;
}
...
exports.extractProjection = extractProjection;
async function parseInsertObj(col, obj) {
    const def = col.def, fields = await col.fieldsFor(obj), insertObj = new col();
    _.each(col.denormal, function (field, name) {
        // TODO:  need to parse & process name if it is a path (if it contains "."s)
        name = namePath_1.default.populateNameFor(name, true);
...
exports.parseInsertObj = parseInsertObj;
function parseProjection(col, obj) {
    const def = col.def, projection = Object.assign({}, obj);
...
function evaluateClient(client, key, doc, value, opts, proj) {
...
exports.evaluateClient = evaluateClient;
function toClient(col, doc, opts) {
    ...
    _.each(doc, (v, k) => {
        ...
        else if (v && mongodb_1.ObjectId.isValid(v.toString())) {
            obj[k] = v.toString();
        }
        else {
    ...
    _.each(fields, function (field, name) {
        const fieldDef = field.def, getFn = fieldDef.get;
        if (!getFn) {
            return;
        }
        if (!evaluateClient(fieldDef.client, name, doc, 
