...
const populator_1 = require("../core/populator");
const ArrayType = new type_1.default({
    name: 'array',
    compile(compiler, field) {
        compiler.type(field, 'of', true);
    },
    fromClient(field, value) {
        const ofField = field.of, ofFieldType = ofField.type;
...
tyr_1.default.mongoCompare = mongoCompare;
function inverseMongoCompare(a, b) {
    return -1 * mongoCompare(a, b);
}
tyr_1.default.arraySort = function (array, sortObj) {
    const sortProps = [];
...
tyr_1.default._slice = async function (doc, path, opts) {
    const col = doc.$model, field = col.paths[path], np = field.namePath;
    const { skip, limit, sort, where, populate } = opts || {};
    const arrDoc = await col.byId(doc.$id, { fields: { [path]: 1 } });
    let arr = np.get(arrDoc);
    if (populate) {
        const populator = new populator_1.default(false /* TODO: ??? */), population = population_1.default.parse(populator, np, populate);
    ...
    if (sort) {
        tyr_1.default.arraySort(arr, sort);
    }
    ...
    const begin = skip || 0, end = limit ? Math.min(begin + limit, arr.length) : arr.length;
