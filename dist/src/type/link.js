...
const LinkType = new type_1.default({
    name: 'link',
    ...
    fromClient(field, value) {
        const link = field.link, linkField = link.fields[field.link.def.primaryKey.field];
        try {
    ...
    toClient(field, value) {
        return value ? (mongodb_1.ObjectId.isValid(value.toString()) ? value.toString() : value) : value;
    },
    format(field, value) {
...
collection_1.default.prototype.links = function (search) {
    search = search || {};
...
collection_1.default.prototype.references = async function (opts) {
    const thisCol = this, refs = [];
