...
const isArray = Array.isArray, O_DELETE = 0, O_TRUNCATE_ARRAY_BY_1 = 1, T_ARRAY = 0, T_OBJECT = 1;
function badPatch() {
...
function diffObj(a, b, props) {
    ...
    for (const prop in a) {
        ...
        if (!tyr_1.default.isEqual(av, bv)) {
            if (bv === undefined) {
                diffs[prop] = O_DELETE;
            }
            else {
                ...
                else if (tyr_1.default.isObject(av) && tyr_1.default.isObject(bv)) {
...
function patchObj(a, patch, props) {
    for (let prop in patch) {
        ...
        if (pv === O_DELETE) {
            ...
            let arr;
            if (prop.indexOf('|')) {
                prop = tyr_1.default.NamePath.decode(prop);
            }
            if (prop.indexOf('.') < 0) {
                arr = a[prop];
            }
            ...
            if (!arr.length) {
                tyr_1.default.warn({ e: 'historical', m: `the array at path "${prop}" in the patch is already empty` }, new Error());
                continue;
            }
            arr.length--;
            continue;
            ...
            switch (pv.length) {
                case 1:
                    ...
                    a[prop] = tyr_1.default.cloneDeep(tyr_1.default.parseBson(pv[0]));
                    continue;
                case 2:
...
function diffArr(a, b) {
    const alen = a.length, blen = b.length, diff = {};
    a = _.clone(a);
    for (let bi = 0; bi < blen; bi++) {
        const bv = b[bi];
        // if the same object is in same place, skip to next one
        if (bi < alen && tyr_1.default.isEqual(bv, a[bi])) {
...
function patchArr(a, patch) {
    ...
    for (const pn in patch) {
        const pi = parseInt(pn, 10), pv = patch[pn];
        if (isNaN(pi)) {
            if (pn === 'n') {
                n = pv;
            }
            ...
            if (isArray(pv)) {
                switch (pv.length) {
                    case 1:
                        a[pi] = tyr_1.default.cloneDeep(tyr_1.default.parseBson(pv[0]));
                        continue;
                    case 2:
                        const [offset, len] = pv;
                        const plen = pi + len;
