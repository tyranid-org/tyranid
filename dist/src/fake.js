...
function fakeField(field) {
    ...
    if (!def)
        throw new Error('No field.def property to fake on! ' + JSON.stringify(field));
    switch (def.name) {
        case 'integer':
        case 'float':
        case 'double':
        ...
        case 'mongoid':
            let i = 24, s = '';
            while (i--)
                s += faker.random.number(15).toString(16);
            return ObjectId(s);
        case 'boolean':
...
collection_1.default.prototype.fake = async function ({ n, schemaOpts, seed } = {}) {
    // get doc schema
    const collection = this, schema = await collection.fieldsFor(schemaOpts);
    ...
    if (n === undefined) {
        ...
        const out = [];
        while (n--)
            out.push(new collection(fakeDocument(schema)));
        return out;
    }
};
