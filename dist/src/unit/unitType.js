...
const U = require("./unitUtil");
const UnitType = new collection_1.default({
    id: '_u1',
    name: 'unitType',
    enum: true,
    client: false,
...
function addComponent(components, component) {
    const type = component.type, degree = component.degree;
...
function baseify(components) {
    for (let i = 0; i < components.length;) {
        const ci = components[i], cti = ci.type;
        if (!cti.abbreviation) {
...
tyr_1.default.UnitType = UnitType;
