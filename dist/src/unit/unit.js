...
const Unit = new collection_1.default({
    id: '_u2',
    name: 'unit',
    enum: true,
    client: false,
...
Unit.boot = function (stage, pass) {
    const UnitType = tyr_1.default.UnitType, UnitSystem = tyr_1.default.UnitSystem;
    if (bootNeeded && UnitType && UnitType.def.values) {
        ...
        for (const unit of units) {
            if (unit.formula) {
                unit.units = tyr_1.default.Units.parse(unit.formula);
            }
            if (unit.type) {
...
Unit.parse = function (name) {
    const UnitFactor = tyr_1.default.UnitFactor;
    let u = bySymbol[name];
    if (u) {
        return u;
    }
