...
class Units {
    constructor(sid, components) {
        ...
        Object.defineProperty(this, 'type', {
            value: tyr_1.default.UnitType.byComponents(typeComponents)
        });
    }
...
Units.parse = function (text) {
    ...
    const len = text.length, components = new Array(compCount);
    let multiplier = 1, pos = 0, nextComp = 0;
    ...
    while (pos < len) {
        ...
        const unit = tyr_1.default.Unit.parse(name);
        if (!unit) {
...
tyr_1.default.U = Units.parse.bind(Units);
//
//  Unit Conversion
//
...
const INCOMPATIBLE = 'INCOMPATIBLE', MANUAL = 'MANUAL';
...
function multAdd(len, components, addComponents, mult) {
    for (const addComp of addComponents) {
        const addUnit = addComp.unit, addDegree = addComp.degree * mult;
        let ci = 0;
        for (; ci < len; ci++) {
            const comp = components[ci];
...
Units.prototype.toString = function () {
    let str = '', positiveFound = false, inverse = false, floatingOne = false;
...
tyr_1.default.UnitDegree = UnitDegree;
tyr_1.default.Units = Units;
