...
function NamePath(base, pathName, skipArray) {
    ...
    const path = this.path = pathName.length ? pathName.split('.') : [], plen = path.length, pathFields = this.fields = new Array(plen);
    let at = base;
    let pi = 0, denormal;
        ...
        if (name === '_') {
            ...
            if (!at.fields) {
                ...
                if (aAt && aAt.fields && aAt.fields[name]) {
                    ...
                    if (!def.keys) {
                        throw new Error(`"${name}" in "${pathName}" is not valid` +
...
NamePath.prototype.toString = function () {
    return ((this.base instanceof Tyr.Collection
        ? (this.base.def.name + ':')
        : (this.base.toString() + '/'))
        + this.name);
};
...
NamePath.prototype.get = function (obj) {
    const np = this, path = np.path, fields = np.fields, plen = path.length;
...
exports.default = NamePath;
//# sourceMappingURL=namePath.js.map