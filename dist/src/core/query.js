...
//
function isValue(v) {
    return !_.isObject(v) || (v instanceof mongodb_1.ObjectId);
}
...
function mergeOpObject(v1, v2) {
    const o = {};
    const o1 = isOpObject(v1) ? v1 : { $eq: v1 }, o2 = isOpObject(v2) ? v2 : { $eq: v2 };
    //
        ...
        if (v1 === undefined) {
            ...
            switch (op) {
                ...
                case '$nin':
                    ...
                    break;
                case '$eq':
                    if (tyr_1.default.isEqual(v1, v2)) {
                        o.$eq = v1;
                    }
                    else {
                        return false;
...
function simplifyOpObject(o) {
    ...
    if (eqv && nev) {
        if (tyr_1.default.isEqual(eqv, nev)) {
            return false;
        }
        delete o.$ne;
    }
...
function merge(query1, query2) {
    ...
    if (tyr_1.default.isEqual(query1, query2)) {
    ...
    for (const n in query1) {
        const v1 = query1[n], v2 = query2[n];
        if (!v2 || tyr_1.default.isEqual(v1, v2)) {
            query[n] = simplify(v1);
            continue;
        }
        else if (!v1) {
...
function valueMatches(match, value) {
    if (isValue(match)) {
        return tyr_1.default.isEqual(match, value);
    }
    for (const op in match) {
        if (!op.startsWith('$')) {
            ...
            switch (op) {
                case '$eq':
                    if (!tyr_1.default.isEqual(match.$eq, value)) {
                        return false;
                    }
                    break;
                case '$in':
...
//
collection_1.default.prototype.fromClientQuery = function (query) {
    const col = this;
