    ...
    if (indexes && col.db) {
        ...
        for (const index of indexes) {
            ...
            if (keyHash in existingIndexesByKey && nameHash in existingIndexesByName) {
                ...
                if (existingIndexesByKey[keyHash] === existingIndexesByName[nameHash]) {
                    ...
                    indexCreationConflict(index, existingIndexesByKey[keyHash], existingIndexesByName[nameHash]);
                }
            }
            else {
        ...
        for (const index of create) {
            ...
            if (createIndexesByKey[key] !== index) {
                throw new Error(`Tried to create two indexes with key = ${JSON.stringify(index.key, null, 2)} for collection ${col.def.name}`);
            }
        }
        const remove = existingIndexes.filter(i => !alwaysInclude.has(toName(i)) &&
            !keep.some(k => k === i));
...
tyr_1.default.createIndexes = async function () {
