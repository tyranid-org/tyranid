...
const { collections, labelize } = tyr_1.default;
...
async function postFind(collection, opts, documents) {
        ...
        if (asOf) {
            if (array) {
                for (const document of documents) {
                    historical_1.default.asOf(collection, document, asOf, opts.fields);
                }
            }
            else {
                historical_1.default.asOf(collection, documents, asOf, opts.fields);
            }
        }
    ...
    if (array) {
        await tyr_1.default.Event.fire({ collection, type: 'find', when: 'post', _documents: documents, opts });
    }
    else {
        await tyr_1.default.Event.fire({ collection, type: 'find', when: 'post', document: documents, opts });
    }
    if (opts.plain) {
        if (array) {
...
function timestampsUpdate(opts, collection, update, doc) {
    if (collection.def.timestamps && opts.timestamps !== false) {
        ...
        if (common_1.hasMongoUpdateOperator(update)) {
...
async function preSave(collection, obj, opts) {
    if (!opts || !opts.preSaveAlreadyDone) {
        const vFields = collection.validatedFields, denormal = collection.denormal;
        let promises, pi = 0;
...
class Collection {
    constructor(def) {
        ...
        if (tyr_1.default.byId[colId]) {
            throw new Error(`Collection ${def.name}: The collection id "${colId}" is already in use by ${tyr_1.default.byId[colId].def.name}.`);
        }
        //
      ...
      CollectionInstance._wrap(this, data);
        ...
        type_1.default.validateType(CollectionInstance);
        if (!def.dbName) {
            def.dbName = def.name;
        }
        if (!def.primaryKey) {
        ...
        const db = def.db || tyr_1.default.options.db;
        if (db) {
            CollectionInstance.db = db.collection(CollectionInstance.def.dbName);
        }
        collections.push(CollectionInstance);
        tyr_1.default.components.push(CollectionInstance);
        tyr_1.default.byId[def.id] = CollectionInstance;
        tyr_1.default.byName[def.name] = CollectionInstance;
        for (const key in dp) {
        ...
        document_1.defineDocumentProperties(dp);
        _.each(def.fields, function (fieldDef, name) {
            const get = fieldDef.getServer || fieldDef.get, set = fieldDef.setServer || fieldDef.set, fn = fieldDef.fn || fieldDef.fnClient || fieldDef.fnServer, isDb = fieldDef.db;
            if (fn) {
    ...
    _wrap(doc, pojo) {
        if (pojo) {
            ...
            if (pojo._id && this.def.historical) {
                historical_1.default.preserveInitialValues(this, doc);
            }
        }
    }
    get label() {
    ...
    async byIds(ids, options) {
        ...
        if (collection.isStatic()) {
            ...
            const opts = common_1.combineOptions(options, { query: { [idFieldName]: { $in: ids } } });
    ...
    byLabel(n, forcePromise) {
        const collection = this, findName = collection.labelField.path, matchLower = n.toLowerCase();
        if (collection.isStatic()) {
            ...
            const query = {};
            query[findName] = { $regex: common_1.escapeRegex(matchLower), $options: 'i' };
            return collection.findOne(query);
        }
    }
    /** @isomorphic */
    labelFor(doc) {
        const collection = this, labelField = collection.labelField;
        if (!labelField) {
    ...
    find(...args) {
        const collection = this, opts = common_1.extractOptions(collection, args), db = collection.db;
        if (args.length && (args.length !== 1 || (!common_1.isOptions(args[0]) && _.keys(args[0]).length))) {
        ...
        fields = opts.fields;
        if (fields) {
            opts.fields = fields = common_1.parseProjection(collection, fields);
        }
        const auth = common_1.extractAuthorization(opts);
        function cursor() {
        ...
        if (auth) {
            return tyr_1.default.mapAwait(collection.secureFindQuery(query, opts.perm || OPTIONS.permissions.find, auth), securedQuery => {
    ...
    async findAll(...args) {
        ...
        const cursor = await this.find(...args), documents = await cursor.toArray();
     ...
     */
    async findOne(...args) {
        const collection = this, db = collection.db;
        let opts = common_1.extractOptions(collection, args);
        switch (args.length) {
            case 2:
                const f = args[1];
                if (f) {
            ...
            case 1:
                ...
                if (args[0] instanceof mongodb_1.ObjectId) {
        ...
        let projection = common_1.extractProjection(opts);
        if (projection) {
            projection = common_1.parseProjection(collection, projection);
        }
        const auth = common_1.extractAuthorization(opts);
    ...
    async findAndModify(opts) {
        const collection = this, db = collection.db, auth = common_1.extractAuthorization(opts);
        if (auth) {
        ...
        if (collection.def.historical && opts.historical !== false) {
            tyr_1.default.warn('findAndModify() used on historical collection', new Error());
        }
        if (update) {
            ...
            if (opts.upsert) {
                const setOnInsertSrc = replaceEntireDoc ? update : update.$setOnInsert, $setOnInsert = await common_1.parseInsertObj(collection, _.merge(_.cloneDeep(opts.query), setOnInsertSrc));
                if (replaceEntireDoc) {
        ...
        opts.fields = common_1.extractProjection(opts);
        if (opts.fields) {
            opts.fields = common_1.parseProjection(collection, opts.fields);
        }
        if (!opts.eventAlreadyDone) {
            await tyr_1.default.Event.fire({ collection, type: 'update', when: 'pre', opts });
        }
        const result = await db.findAndModify(opts.query, opts.sort, opts.update, opts);
        if (result && result.value) {
            result.value = new collection(result.value);
            if (!opts.eventAlreadyDone) {
                await tyr_1.default.Event.fire({ collection, type: 'update', when: 'post', opts });
            }
        }
        else {
            if (!opts.eventAlreadyDone) {
    ...
    count(opts) {
        const collection = this, db = collection.db, query = opts.query, auth = common_1.extractAuthorization(opts);
        if (auth) {
            return tyr_1.default.mapAwait(collection.secureFindQuery(query, opts.perm || OPTIONS.permissions.find, auth), securedQuery => db.count(securedQuery));
        }
    ...
    async exists(opts) {
        const collection = this, db = collection.db, query = opts.query, auth = common_1.extractAuthorization(opts);
        if (auth) {
            return tyr_1.default.mapAwait(collection.secureFindQuery(query, opts.perm || OPTIONS.permissions.find, auth), async (securedQuery) => !!(await db.findOne(securedQuery, { _id: 1 })));
        }
    ...
    async save(obj, opts) {
        ...
        if (Array.isArray(obj)) {
            const arrOpts = common_1.combineOptions(opts, { preSaveAlreadyDone: true });
            // TODO:  use bulkops
            ...
            if (collection.def.historical && (!opts || opts.historical !== false)) {
                if (obj.$historical) {
                    ...
                    historical_1.default.snapshot(collection, obj, historical_1.default.patchPropsFromOpts(opts));
                }
            }
            const keyFieldName = collection.def.primaryKey.field, keyValue = obj[keyFieldName];
            let rslt;
            if (keyValue) {
                await tyr_1.default.Event.fire({ collection, type: 'update', when: 'pre', document: obj, opts });
                // using REPLACE semantics with findAndModify() here
                const result = await collection.findAndModify(common_1.combineOptions(opts, {
                ...
                await tyr_1.default.Event.fire({ collection, type: 'update', when: 'post', document: obj, opts });
                rslt = result.value;
            }
            else {
                const modOpts = common_1.combineOptions(opts, { preSaveAlreadyDone: true });
    ...
    async insert(obj, opts) {
        ...
        const auth = common_1.extractAuthorization(opts);
        if (Array.isArray(obj)) {
            const parsedArr = await Promise.all(_.map(obj, el => common_1.parseInsertObj(collection, el)));
            if (auth) {
                const canInsertArr = await Promise.all(parsedArr.map(parsedObj => collection.canInsert(parsedObj, opts.perm || OPTIONS.permissions.insert, auth)));
                if (canInsertArr.some(val => !val)) {
                    // TODO:  throw a security exception here ?
                    return false;
                }
            }
            await tyr_1.default.Event.fire({ collection, type: 'insert', when: 'pre', _documents: parsedArr, opts });
            const docs = parsedArr.length ? (await collection.db.insertMany(parsedArr)).ops : [];
            await tyr_1.default.Event.fire({ collection, type: 'insert', when: 'post', _documents: docs, opts });
            return docs;
        }
        else {
            const parsedObj = await common_1.parseInsertObj(collection, obj);
            if (auth) {
            ...
            await tyr_1.default.Event.fire({ collection, type: 'insert', when: 'pre', document: parsedObj, opts });
            const rslt = await collection.db.insert(parsedObj);
            const doc = rslt.ops[0];
            obj._id = doc._id;
            await tyr_1.default.Event.fire({ collection, type: 'insert', when: 'post', document: doc, opts });
            return doc;
        }
    }
    /**
    ...
    async updateDoc(obj, ...args) {
        const collection = this, keyFieldName = collection.def.primaryKey.field;
        if (!obj[keyFieldName]) {
        ...
        const opts = common_1.combineOptions(common_1.extractOptions(collection, args), {
            query: { [keyFieldName]: obj[keyFieldName] },
            upsert: true,
            new: true
        });
        const updateFields = common_1.extractUpdateFields(obj, opts), update = {};
        ...
        if (collection.def.historical && opts.historical !== false) {
            ...
            const snapshot = historical_1.default.snapshot(collection, obj, historical_1.default.patchPropsFromOpts(opts), updateFields, historyPresent);
            if (snapshot) {
                if (historyPresent) {
    ...
    async update(...args) {
        const collection = this;
        const opts = common_1.extractOptions(collection, args);
        switch (args.length) {
            case 2:
                opts.update = args[1];
            // fall through
        ...
        if (collection.def.historical && opts.historical !== false) {
            tyr_1.default.warn('update() (not $update()) used on historical collection', new Error());
        }
        const update = opts.update, auth = common_1.extractAuthorization(opts);
        let query = opts.query;
        if (auth) {
            query = await collection.secureQuery(query, opts.perm || OPTIONS.permissions.update, auth);
            if (!query) {
        ...
        await tyr_1.default.Event.fire({ collection, type: 'update', when: 'pre', query, opts });
        const rslt = await collection.db.update(query, update, opts);
        await tyr_1.default.Event.fire({ collection, type: 'update', when: 'post', query, opts });
        return rslt;
    }
    async pull(id, path, predicate, ...args) {
        const collection = this, opts = common_1.extractOptions(collection, args), np = collection.parsePath(path);
        const qOpts = tyr_1.default.cloneDeep(opts);
    ...
    async push(id, path, value, ...args) {
        const collection = this, opts = common_1.extractOptions(collection, args), auth = common_1.extractAuthorization(opts), query = { _id: id };
        if (auth) {
            query = await collection.secureQuery(query, opts.perm || OPTIONS.permissions.update, auth);
            if (!query) {
                throw new secureError_1.default();
            }
        }
        const pv = {
            [path]: value
        ...
        if (collection.def.historical && opts.historical !== false && collection.parsePath(path).isHistorical()) {
            pv._history = historical_1.default.snapshotPush(path, historical_1.default.patchPropsFromOpts(opts));
        }
     ...
     */
    async remove(...args) {
        const collection = this, opts = common_1.extractOptions(collection, args);
        ...
        if (justOne !== '$remove') {
            await tyr_1.default.Event.fire({ collection, type: 'remove', when: 'pre', query, opts });
        }
        return await collection.db.remove(query, justOne);
        if (justOne !== '$remove') {
            await tyr_1.default.Event.fire({ collection, type: 'remove', when: 'post', query, opts });
        }
    }
    /**
    ...
    populate(fields, documents, denormal, opts) {
        const collection = this, populator = new populator_1.default(denormal, opts), population = population_1.default.parse(populator, collection, fields);
    ...
    fromClient(pojo, path, opts) {
        let collection = this, fields = collection.fields;
        ...
        if (namePath) {
            ...
            collection = detail.def.id ? tyr_1.default.byId[detail.def.id] : null;
     ...
     */
    toClient(data, opts) {
        return common_1.toClient(this, data, opts);
    }
    parsePath(path) {
        return new namePath_1.default(this, path);
    }
            ...
            type(field, name, required) {
                ...
                if (!type) {
                    ...
                    if (_.isPlainObject(type)) {
                        type = field[name] = new field_1.default(type);
                    }
                    else if (_.isString(type)) {
                        type = type_1.default.byName[type];
                        if (!type) {
                            if (this.stage === 'link') {
                            ...
                            type = field[name] = new field_1.default({ is: type.def.name });
                        }
                    }
                    else {
                ...
                if (type instanceof field_1.default) {
            ...
            field(path, field) {
                ...
                if (linkDef) {
                    if (_.isString(linkDef)) {
                        ...
                        type = type_1.default.byName[linkDef];
                    }
                    else {
                        type = linkDef;
                    }
                    if (type || stage === 'link') {
                        ...
                        field.link = type;
                        field.type = type_1.default.byName.link;
                    ...
                    if (!type) {
                        type = fieldDef.is;
                        if (_.isString(type)) {
                            type = type_1.default.byName[fieldDef.is];
                            if (type instanceof Collection) {
                ...
                for (const fnName of ['get', 'set', 'getClient', 'setClient', 'getServer', 'setServer', 'validate', 'where']) {
                    if (fieldDef[fnName] && common_1.isArrow(fieldDef[fnName])) {
            ...
            fields(path, parent, defFields) {
                ...
                _.each(_.keys(defFields), name => {
                    ...
                    if (!(field instanceof field_1.default)) {
                        field = defFields[name] = new field_1.default(field);
                    }
                    // if it is an optional link to a collection that does not exist, prune it from the definition
                    let link;
                    if (stage === 'link' && (link = field.def.link) && _.isString(link) && link.endsWith('?') && !type_1.default.byName[link.substring(0, link.length - 1)]) {
    ...
    compile(stage) {
        ...
        for (const fnName of ['fromClient', 'toClient']) {
            if (collection.def[fnName] && common_1.isArrow(collection.def[fnName])) {
        ...
        if (stage === 'link') {
            if (collection.def.historical) {
                historical_1.default.link(collection);
            }
            const validatedFields = collection.validatedFields = [], paths = collection.paths;
    ...
    validateValues() {
        const collection = this, def = collection.def, rows = def.values;
        if (!rows) {
            return;
        }
        ...
        if (Array.isArray(rows[0])) {
            ...
            const header = rows[0], hlen = header.length, newValues = [];
            let hi, name;
...
tyr_1.default.mixin(Collection, component_1.default);
tyr_1.default.Collection = Collection;
